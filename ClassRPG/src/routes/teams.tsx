...
function TeamsPage() {
  ...
  const me = data.students.find((s) => s.id === user?.id) ?? data.currentStudent;
  const teams = data.teams;
  if (teams.length === 0) {
    return (
      <AppShell role="student" title="Equipes">
        <ErrorState label="Nenhuma equipe cadastrada ainda." />
      </AppShell>
    );
  }
  const myTeam = teams.find((t) => t.id === me?.teamId) ?? teams[0];
  const ranking = [...teams].sort((a, b) => b.xp - a.xp);
  const myTeamMembers: Student[] = data.students
