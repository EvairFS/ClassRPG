...
function RankingPage() {
  ...
  const sorted: Student[] = data.map((r) => r.student);
  const top3 = sorted.slice(0, 3);
  const myIndex = user ? sorted.findIndex((s) => s.id === user.id) : -1;
    ...
    <AppShell role="student" title="Ranking">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_320px]">
          <RankingTable students={sorted} currentUserId={user?.id} />
          <aside className="glass relative overflow-hidden rounded-2xl p-5">
            ...
            <p className="mt-1 text-4xl font-bold text-foreground">{me ? `#${myIndex + 1}` : "—"}</p>
            ...
            <div className="mt-4 space-y-2 text-sm">
              <div className="flex justify-between text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">{(me?.xp ?? 0).toLocaleString("pt-BR")}</span>
              </div>
              <div className="flex justify-between text-muted-foreground">
                <span>Falta para o top 3</span>
                <span className="font-semibold text-accent">
                  +{Math.max(0, (top3[2]?.xp ?? 0) - (me?.xp ?? 0) + 1).toLocaleString("pt-BR")} XP
                </span>
              </div>
            </div>
...
function PodiumCard({ student, place }: { student: Student; place: number }) {
  const meta = [
